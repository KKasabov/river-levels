...
module.exports = {
  getLatestLocalReading: function(dev_id) {
    return new Promise(function(resolve, reject) {
      var params = [dev_id];
      var sql = "SELECT timestamp, waterLevel FROM ni60.localDataLog WHERE devID = ? ORDER BY timestamp DESC LIMIT 1";
  ...
  insertLocalDataRecord: function(params) {
    var sqlSelect = "SELECT DATE_FORMAT( MAX(timestamp), '%Y-%m-%dT%TZ') AS timestamp FROM ni60.localDataLog WHERE devID = ?";
    conn.query(sqlSelect, params.devID, function(err, result) {
      if (err) throw err;
      if (result[0].timestamp != params.timestamp) {
        var sqlInsert = "INSERT INTO ni60.localDataLog SET ?";
  ...
  getLatestEnvAgencyReading: function(stationReference) {
    return new Promise(function(resolve, reject) {
      ...
      var sql = "SELECT timestamp, readingValue FROM ni60.envAgencyDataLog WHERE stationReference = ? ORDER BY timestamp DESC LIMIT 1";
      conn.query(sql, params, function(err, result) {
        if (err) return reject(err);
        resolve(result);
      });
